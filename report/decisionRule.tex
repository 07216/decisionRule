...
It's important to notice that actually $x_{t,j}(\xi^{t-1},\xi_t) = u(\xi^{t-1}) \bigwedge \xi_t$ and equation \ref{origin} can be reformulated as the following equation \ref{originsec} according to Preservation of Structural Properties in Optimization
...
s.t. & \sum\limits^T_{t=1} A \mathbf{\mathcal{X}_t}\left(\xi_t\right) \leq \mathbf{c}\\
&0 \leq \mathcal{X}_t(\xi_t) \leq \xi_t\\
& \mathbf{\mathcal{X}_t}(\xi_t) = (\mathcal{X}_{t,1}(\xi_{t,1}),\mathcal{X}_{t,2}(\xi_{t,2})\ldots,\mathcal{X}_{t,j}(\xi_{t,j}))^T \in \mathcal{F}(\xi^{t-1})\\
&\forall \xi \in \Xi,t=1,\ldots,T
\end{array}
\label{originsec}
\end{equation}
where $\mathcal{F}(\xi^{t-1})=\{f(\xi_t):f(\xi_t) = u(\xi^{t-1}) \bigwedge \xi_t\}$.

\section{Linear Decision Rule Approach via Lifting}
\subsection{Primal Problem}

Approach original problem with $$x_{t,j}(\xi^{t-1},\xi_{t,j})=(X_tP_tL(\xi))_j+\tilde{X}_{t,j}Q_{t,j}L(\xi)$$ where
...
and
$$
L(\xi^{t-1})=(L_{\tilde{t},j}(\xi_{\tilde{t},j}))_{\tau\times j,1}
$$
...
The correction $\mathbf{c} - \mathbb{E}_{\xi}\left(\sum^T_{t=1} A (X_t P_t + \sum_{k=1}^j\tilde{X}_{t,k}Q_{t,k})\xi\right) $arises from inaccurately approaching $\mathbf{x}(\xi^{t-1},\xi_t)$.
...
As we can see from Preservation of Structural Properties in Optimization with Decisions Truncated by Random Variables and Its Applications, $\tilde{X}_{t,j}(\xi^{t-1},\xi_{t,j})$ should behave $u\bigwedge \xi_{t,j}$. Benefiting from piecewise linear function, the structure is preserved with linear decision rule. Therefore, it's convenient to implement the policy as we can get $u$ from $\tilde{X}_{t,j}$.
